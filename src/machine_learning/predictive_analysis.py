...
def resize_input_image(img, version):  
    ...
    """
    image_shape = load_pkl_file(file_path=f"{current_dir}/outputs/{version}/image_shape.pkl")
...
def load_model_and_predict(my_image, version):
    ...
    model = load_model(f"{current_dir}/outputs/{version}/detection_of_deseased_leaves_model.h5")
    ...
    st.write(
        f"The predictive analysis indicates that the sample leaf is "
        f"**{pred_class.lower()}**.")
